  ...
  return (
    <div className="bg-gradient-to-br from-[#817962] via-[#a2a099] to-[#c5c0ab] shadow-md w-full max-w-96 transition-transform duration-300 hover:scale-105 hover:shadow-xl p-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-bold text-lg text-[#5c260b]">{spec.manufacturer} {spec.model}</h3>
      ...
      <div className="flex flex-wrap justify-between text-sm text-gray-600 mt-3">
        <div className="flex items-center gap-1 mb-2"><GiGearStickPattern />{spec.transmission}</div>
        <div className="flex items-center gap-1 mb-2"><PiEngineLight />{spec.engineCapacity}</div>
        <div className="flex items-center gap-1 mb-2"><BsPeopleFill />{spec.seatingCapacity} seater</div>
        <div className="flex items-center gap-1 mb-2"><GiGasPump />{spec.fuelType}</div>
      </div>

      {/* Book/Login */}
      {isAuthenticated ? (
        <button
          className="bg-[#888776] w-full mt-4 py-2 rounded-md text-[#161135] transition-transform duration-300 hover:scale-105 hover:shadow-xl font-bold"
          onClick={() =>
            navigate("/booknow", {
              state: { car, userId: user._id },
            })
          }
        >
          Book Now
